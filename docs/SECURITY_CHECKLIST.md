...
- [x] 메모 수정/삭제 소유권 검증 (`X-User-ID` 인가, repository 의 user_id 조건)
...
## 코드 상태 기준 (origin/main = 7773c60, 5단계 후, 운영 미배포)
...
## 인증 vs 인가 (한계 인정)
- [x] **인가** (Authorization): `X-User-ID` 가 식별하는 사용자의 자원만 접근 — 구현됨
- [ ] **인증** (Authentication): "이 사용자가 진짜 그 사용자인가" 검증 — 부재 (backlog)
   - 현재 `X-User-ID` 는 클라이언트 주장값
   - JWT + bcrypt + refresh token rotation 도입 예정 (auth plugin 한 곳만 변경)
...
- [ ] multipart register 위치를 auth plugin 안으로 이동 (5단계 narrative cleanup 후 발견된 DoS 위험, 다음 배포 사이클로)
